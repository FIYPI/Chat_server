...
DEFINE_string(access_host, "127.0.0.1:7070", "当前实例外部访问地址");
DEFINE_int32(liscen_port, 7070, "Rpc服务监听端口");
...
int main(int argc, char* argv[])
{
    ...
    ret = server.Start(FLAGS_liscen_port, &options);
    if (ret == -1) {
        std::cout << "启动服务器失败！\n";
        return -1;
    }
    ...
    rclient->serviceRegistry(FLAGS_base_service + FLAGS_instance_name, FLAGS_access_host);
